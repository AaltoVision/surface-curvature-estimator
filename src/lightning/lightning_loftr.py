...
from src.DPT.depth_estimation import MiDasDepth
...
class PL_LoFTR(pl.LightningModule):
    def __init__(self, config, pretrained_ckpt=None, profiler=None, dump_dir=None):
        ...
        # depth_estimation amd curvature extraction
        self.pred_depth = MiDasDepth(_config)

        # Pretrained weights
        if pretrained_ckpt:
            state_dict = torch.load(pretrained_ckpt, map_location='cpu')['state_dict']
            if _config['trainer']['testing']:
    ...
    def optimizer_step(
        ...
        # update params
        # a = list(self.parameters())[-3].clone()
        optimizer.step(closure=optimizer_closure)
        optimizer.zero_grad()
        # b = list(self.parameters())[-3].clone()
        # print('params mean:', a.data.mean(), b.data.mean())
        # print('if params are the same:', torch.equal(a.data, b.data))
    ...
    def _trainval_inference(self, batch):
        ...
        # compute the curvature at the coarse level
        self.pred_depth(batch)
