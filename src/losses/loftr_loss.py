...
from src.DPT.midas.midas_loss import MidasLoss
...
class LoFTRLoss(nn.Module):
    def __init__(self, config):
        ...
        self.depth_loss = MidasLoss(alpha=0.1)
    ...
    def compute_depth_loss(self, depth0, depth1, depth0_gt, depth1_gt):
        mask_valid = torch.ones_like(depth0).long()
        midas_loss0, ssi_loss0, reg_loss0 = self.depth_loss(depth0, depth0_gt.unsqueeze(0), mask_valid)
        midas_loss1, ssi_loss1, reg_loss1 = self.depth_loss(depth1, depth1_gt.unsqueeze(0), mask_valid)
        loss = ssi_loss0 * 0.5 + ssi_loss1 * 0.5
        return loss
    ...
    def forward(self, data):
        ...
        if data.get('pred_depth0') is not None:
            pred_depth0, pred_depth1 = data['pred_depth0'], data['pred_depth1']
            gt_depth0, gt_depth1 = data['depth0'], data['depth1']
            depth_loss = self.compute_depth_loss(pred_depth0, pred_depth1, gt_depth0, gt_depth1)
            loss_scalars.update({"depth_loss": depth_loss.clone().detach().cpu()})
            loss += depth_loss
