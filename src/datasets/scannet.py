...
class ScanNetDataset(utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image1 = read_scannet_gray(img_name1, resize=(640, 480), augment_fn=None)
                                #    augment_fn=np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
        # read image to predict depth
        img2depth0 = read_image_for_depth(img_name0, resize=(640, 480))
        img2depth1 = read_image_for_depth(img_name1, resize=(640, 480))
        ...
        # curv_path0 = osp.join(self.root_dir, scene_name, 'depth', f'{stem_name_0}.curv_est_depth.h5')
        # curv_path1 = osp.join(self.root_dir, scene_name, 'depth', f'{stem_name_1}.curv_est_depth.h5')
        # desc_curv0 = dd.io.load(curv_path0)
        # desc_curv1 = dd.io.load(curv_path1)
