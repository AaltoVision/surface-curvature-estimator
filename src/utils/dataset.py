...
from src.DPT.midas.transforms import Resize, NormalizeImage, PrepareForNet
...
def resize_image_for_depth(img):
    net_w, net_h = 384, 384
    resize_mode="minimal"
...
def read_image_for_depth(path, resize = ((640, 480))):
    ...
    img_input = resize_image_for_depth(img)
...
def read_image_for_depth_megadepth(path, resize=None, df=None, padding=False, augment_fn=None):
    """Read image and output RGB image (0-1).
    ...
    if padding:  # padding
        pad_to = max(h_new, w_new)
        image_after, mask = pad_bottom_right(image, pad_to, ret_mask=True)

    if img.ndim == 2:
        image_after = cv2.cvtColor(image_after, cv2.COLOR_GRAY2BGR)

    image_after = cv2.cvtColor(image_after, cv2.COLOR_BGR2RGB) / 255.0

    img_input = resize_image_for_depth(image_after)
    return img_input
